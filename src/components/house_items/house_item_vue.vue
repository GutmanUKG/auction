      ...
      <div class="item-descr">
        <div class="item-name">
          <router-link :to="{name: `detail_house`, params: {id: data.id}}">
            {{data.name}} , {{data.area}}м2 <template v-if="data.year && data.year != 0">{{data.year}} г.п</template>
          </router-link>
        </div>
        <div class="item-address">
          {{data.city}} <template v-if="data.street">{{data.street}}</template> <template v-if="data.address">{{data.address}}</template>
        </div>
        ...
        <div class="item-footer">
          ...
          <div class="item-people">
            ...
            <template v-if="activePeople < data.userCount">
              {{data.userCount}}
            </template>
            <template v-else>
              {{activePeople}}
